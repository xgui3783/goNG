...
func NewInputInterface() *InputInterface {
	...
	OptOutFlags := make(map[string]bool)

	ii := InputInterface{}

	ii.OptOutFlags = &OptOutFlags
...
func (ii *InputInterface) OptOut(flag string) {
	(*ii.OptOutFlags)[flag] = true
}
...
func (ii *InputInterface) SetupFlag(fs *flag.FlagSet) {
	...
	if _, found := (*ii.OptOutFlags)["OutFormat"]; !found {
